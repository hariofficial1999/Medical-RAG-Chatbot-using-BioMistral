...
# ─── API Key (from environment or session state) ───────────────────
...
@st.cache_resource(show_spinner=False)
def build_index_from_pdf(pdf_path, chunk_size=500, chunk_overlap=50):
    """Build FAISS index from a PDF file."""
...
def stream_gemini(prompt: str, api_key: str):
